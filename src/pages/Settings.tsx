...
import { getProfile, updateProfile, uploadImage } from '../api/helper';
...
const Settings = () => {
  ...
  const uploadsImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const FileUpload = await uploadImage(formData);

      console.log('File uploaded successfully:', FileUpload);
      setImgUrl(FileUpload.data.data);
  ...
  const handelSubmit =async()=>{
    try {
      if(!imgUrl){
        return ShowToast("Please select Image")
      }
      const data = {
        image:imgUrl
      }
                    ...
                    <div className="h-14 w-14 rounded-full overflow-hidden">
                      {info && (
                        <img
                          ...
                          src={preview ? preview : info?.image ? info?.image : userThree}
                          alt="User"
                        />
                      )}
                    </div>
